...
static inline char *env_get_value_for_key(env env, char *key) {
    uint32_t caret = 0;
    while (env[caret]) {
        uint32_t key_len = strlen(&env[caret]);
        if (strcmp(&env[caret], key) == 0)
            return &env[caret + key_len + 1];

        caret += key_len + 1 + strlen(&env[caret + key_len + 1]) + 1;
    }
    return (char *)"";
}
...
static inline char *mach_send_message(mach_port_t port,
                                      char *message,
                                      uint32_t len) {
    if (!message || port == MACH_PORT_NULL)
        return NULL;
